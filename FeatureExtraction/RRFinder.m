...
signal(:,1) = double(signalOrig(round(2*Fs):end-round(2*Fs)+1)); %%%% 1st column - ECG signal ; start after 2 seconds
signal(:,1) = signal(:,1)-mean(signal(:,1));

%signal(:,2) = double(signalOrig(round(2*Fs)-delay:end-round(2*Fs)-delay+1,2));
